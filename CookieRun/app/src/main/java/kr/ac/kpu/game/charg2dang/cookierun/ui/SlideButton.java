...
public class SlideButton implements GameObject
{
	...
	{
		this.jump = SharedBitmap.load(R.mipmap.slide_button);
		this.jumpPressed = SharedBitmap.load(R.mipmap.slide_pressed);
		this.x = x;
		this.y = y;
	...
	{
		if(down == true)  jumpPressed.draw(canvs, x, y);
		else jump.draw(canvs, x, y);
	}
